...
| SI-73 | HTTP istemcisi yalnız **incelenmiş dört** modülde; allow-list düz küme değil **`{dizin: {modül}}` haritasıdır** (D'de üçe, G'de dörde genişletildi — daraltılarak) | `technocore/`: `client.py` + `write_client.py` + `evidence_client.py`; `opencode/`: `client.py`; başka hiçbir modül, ve bir dizinde izinli ad başka dizinde izinli değil | `test_write_gate.py::test_httpx_is_imported_only_by_the_reviewed_clients`, `::test_every_reviewed_client_module_actually_exists`, `::test_the_allow_list_is_keyed_by_package_and_not_by_file_name`, `::test_the_write_client_is_not_reachable_from_the_read_path` | A3 → D → G |
...
| SI-213 | Görev katmanı **dördüncü giden yüzey, ikinci kasa/signer veya ikinci gate açmaz** | `modules/`+`tasks/` içinde HTTP istemcisi, `socket`, `urllib`, `station_api.vault` ve `station_api.compose` import'u yok; `tasks.gate.CheckState` **write_gate'inkiyle aynı nesnedir** ve `gate.py` paralel enum tanımlamaz | `test_module_registry.py::test_the_task_layer_has_no_outbound_surface`, `::test_the_task_layer_reaches_no_vault_and_no_signer`, `::test_the_task_gate_reuses_the_write_gates_check_state` | F |
...
| SI-216 | `suggested`, `running` ve `paused` **hiçbir kod yolundan üretilemez**; tanımlı kalırlar | üretici taraması `TaskService`'in **her public metodunu** introspection ile sürer (yalnız `transition`'ı değil) ve durumları doğrudan tablodan okur; ulaşılan küme testte **elle yazılı** `EXPECTED_PRODUCIBLE`'a eşittir — `PRODUCIBLE_STATES` sabiti ayrı satırda denetlenir, böylece sabiti düzenlemek oracle'ı büyütmez; üç duruma doğrudan istek `state_not_producible` ile reddedilir; tablodaki kenarlar durur | `test_task_states.py::test_no_code_path_can_produce_an_unproducible_state`, `::test_the_producer_walk_actually_drives_every_public_method`, `::test_the_service_refuses_a_direct_request_for_an_unbuilt_state`, `::test_the_pure_function_refuses_every_edge_into_an_unbuilt_state`, `::test_the_table_still_lists_the_edges_into_unbuilt_states`, `::test_the_unproducible_states_are_exactly_the_three_the_adr_names` | F |
...
| SI-220 | `public_share` bu sürümde **temsil edilemez**, daima `not_implemented`'tır ve yayımı **engellemez** | `EvidenceRef` yapıcısı reddeder; servis `evidence_field_refused` döner; her şey doğrulanmışken bile alan `not_implemented` ve `ready_to_publish` `True` | `test_task_evidence.py::test_a_public_share_reference_cannot_be_constructed_at_all`, `::test_the_service_refuses_to_record_public_share`, `::test_public_share_is_always_not_implemented_and_never_passed`, `::test_public_share_does_not_block_a_finished_task` | F |
...
| SI-225 | **Bütçe alanı yoktur** ve erteleme belgede görünürdür | görev/registry paketlerinde bütçe biçimli sütun veya tanımlayıcı yok (tek istisna erteleme cümlesi); `budget_available` `Literal[False]`; `docs/task-modules.md` §6 ertelemeyi kaydeder | `test_task_evidence.py::test_the_task_layer_opens_no_budget_field`, `::test_the_deferred_budget_is_stated_rather_than_dropped`, `::test_the_deferral_is_recorded_in_the_documents` | F |
| SI-226 | Bir görevin durumunu yazan **tek** kod yolu `TaskService.transition`'dır | `modules/`+`tasks/` sözdizim ağacında `.state`'e atama (düz, annotate, artırmalı ve sabit adlı `setattr`) yalnız `service.py:transition` içinde; sentetik ikinci yazıcı taramada görünür | `test_task_states.py::test_only_the_transition_method_writes_a_task_state`, `::test_the_state_write_scan_would_see_a_second_writer` | F |
...
kural değil. Fakat liste artık düz bir küme olamazdı: eski yazım
`name in MODULES and parent.name == "technocore"`'du ve `opencode/client.py`
httpx import etseydi **geçerdi** — `client.py` zaten listedeydi. Harita
(`{dizin: {modül}}`) hem bu boşluğu kapatır hem de her giden yüzeyi kendi
dizininde adlandırır.
...
| SI-239 | Anahtar ayrı bir DPAPI zarfındadır, hiçbir tabloya girmez, **sürümlü ve kind denetimlidir**, ve alan ayrımı in-band'dir | zarf `{format, version, kind, created_at, dpapi_blob}`; anahtar dosyada düz değil; yanlış `version`/`kind`/`format` reddedilir; audit materyali zarfı **anahtar diye okunamaz**; ACL yalnız SYSTEM + geçerli kullanıcı | `test_opencode_credentials.py::test_the_envelope_has_the_audit_shape_and_hides_the_credential`, `::test_the_envelope_is_versioned_and_kind_checked`, `::test_an_audit_material_envelope_cannot_be_read_as_a_credential`, `::test_the_acl_is_restricted_to_the_current_user_and_system` | G |
...
| SI-243 | Anahtar canary'si **hiçbir yüzeyde** görünmez: HTTP gövdeleri **ve header'ları**, OpenAPI, SQLite, zarf, veri dizini, log+exception, frontend bundle. Canary depoda başka yerde yoktur | her yüzeyde yok; canary redaksiyon eşiğinin üzerinde ve benzersiz | `test_opencode_leakage.py::test_the_credential_is_absent_from_*` (yedi yüzey), `::test_the_canary_appears_nowhere_else_in_the_repository`, `::test_the_canary_is_long_enough_for_the_redaction_registry_to_hold_it` | G |
