...
| SI-331 | `reasoning_content` **saklanmaz, gösterilmez, loglanmaz** — redakte değil, **düşürülür** | ölçülen yanıttaki alan adı `DISCARDED_MESSAGE_FIELDS` içindedir ve ayrıştırıcıdan çıkan değerde işaretçi **hiçbir alanda** yoktur; bir model turundan sonra hiçbir aktivite satırının `detail`'i işaretçiyi taşımaz; HTTP yanıtının **tamamında** geçmez | `test_model_planner.py::test_the_reasoning_field_is_dropped_rather_than_carried`, `::test_no_timeline_row_from_a_model_turn_carries_the_reasoning`, `test_agent_boundary.py::test_no_agent_table_can_hold_a_model_reasoning_trace` | H4 |
