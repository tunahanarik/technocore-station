...
| SI-216 | `suggested`, `running` ve `paused` **hiçbir kod yolundan üretilemez**; tanımlı kalırlar | gerçek servis gerçek veritabanı üzerinde sürülür, ulaşılan durum kümesi `PRODUCIBLE_STATES`'e **eşittir**; üç duruma doğrudan istek `state_not_producible` ile reddedilir; tablodaki kenarlar durur | `test_task_states.py::test_no_code_path_can_produce_an_unproducible_state`, `::test_the_service_refuses_a_direct_request_for_an_unbuilt_state`, `::test_the_pure_function_refuses_every_edge_into_an_unbuilt_state`, `::test_the_table_still_lists_the_edges_into_unbuilt_states`, `::test_the_unproducible_states_are_exactly_the_three_the_adr_names` | F |
