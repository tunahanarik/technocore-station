...
Durum sütunu bir satırın **hangi pakette** kazanıldığını söyler (A1, A2, A3,
A3.1, B, A2B, C…J); bir ok iki paket arasında **daraltılarak** taşındığını
gösterir. Bekleyen satır yoktur: bu tablodaki her satırın bugün bir kod
yolu ve en az bir testi vardır. Paket J bunu ölçen bir test ekledi —
`test_security_invariants_doc.py::test_every_test_named_in_the_table_exists`.
...
| SI-38 | Seed hiçbir HTTP response, OpenAPI belgesi, log, veri dizini artefaktı veya frontend bundle'ında görünmez (AC-06) | hiçbir yüzeyde yok | `test_seed_leakage.py::test_seed_is_absent_from_every_http_response`, `::test_seed_is_absent_from_the_openapi_document`, `::test_seed_is_absent_from_logs_and_exceptions`, `::test_no_plaintext_artefact_is_left_in_the_data_directory`, `::test_seed_is_absent_from_the_frontend_bundle` | A1 → A2 |
| SI-49 | Seed hiçbir tabloda bulunmaz; ayrı DPAPI zarfındadır | SQLite'ta seed yok, secret sütunu yok, zarf dosyasında düz metin seed yok | `test_seed_leakage.py::test_seed_is_absent_from_the_sqlite_database`, `::test_seed_is_absent_from_the_vault_file`, `::test_passphrases_never_reach_the_database`, `test_database.py::test_no_seed_material_is_written_to_the_database`, `::test_schema_has_no_secret_columns`, `test_identity_vault.py::test_vault_file_contains_no_plaintext_seed` | A2 |
| SI-50 | Paroladan seed türetilmez | parola biçimindeki dosya reddedilir; ne HTTP ne CLI ham seed veya parola argümanı alır | `test_seed_import.py::test_passphrase_form_is_refused`, `::test_no_http_endpoint_accepts_a_raw_seed`, `::test_the_cli_never_takes_a_seed_or_passphrase_argument`, `::test_frontend_has_no_raw_seed_input` | A2 |
| SI-52 | Yanlış parola ve kurcalanmış recovery **aynı** genel hatayı verir | tek hata sözleşmesi; hangi yarının bozulduğu söylenmez | `test_identity_vault.py::test_wrong_passphrase_and_tamper_share_one_error`, `::test_wrong_passphrase_and_ciphertext_tamper_share_one_contract`, `::test_authenticated_header_tampering_is_refused` | A2 |

**SI-50'nin ölçülmeyen yarısı:** `identity/service.py::generate_seed` seed'i
`secrets.token_bytes` ile üretir, ama **bunu doğrudan doğrulayan bir test
yoktur**. Yukarıdaki testler yalnız *içeri alma* yolunu kapatır. Bu boşluk
Paket J'de ölçüldü ve kapatılmadı.
...
> **§9 kaldırıldı (Paket J, ADR-0011 §2).** Başlığı *"Aşama 2+ değişmezleri
> (bugün kod yolu yok)"* idi, ama altındaki sekiz satırın hepsinin bugün bir
> kod yolu var. SI-49/SI-50/SI-52 §6'ya, SI-51/SI-53/SI-54/SI-56 §9c'ye ve
> SI-55 §9e'ye — hepsi test adlarıyla — taşındı. INV-06 "her satır bir
> testle eşleşir" der; test göstermeyen bir satır o kuralın kendisini
> aşındırıyordu.
...
| SI-51 | Restore-test tamamlanmadan hiçbir Technocore write çalışmaz | `recovery_verified` geçmeden kapı kapalı; kapı ön koşulları tek tek sayar | `test_write_gate.py::test_closed_while_recovery_is_pending`, `::test_identity_half_passes_after_a_successful_restore_test`, `::test_unimplemented_requirements_are_never_counted_as_passed`, `::test_every_precondition_together_opens_the_gate`, `::test_no_check_can_be_bypassed_by_a_flag` | A2 → A3 |
| SI-53 | TLS doğrulaması kapatılamaz; host allow-list zorunludur | `verify=` hiçbir yerde geçmez; allow-list dışı her biçim reddedilir | `test_technocore_readonly.py::test_tls_verification_is_never_disabled`, `::test_every_way_around_the_allow_list_is_refused`, `test_write_client.py::test_no_tls_setting_is_passed_anywhere`, `::test_a_transport_with_tls_verification_off_cannot_be_injected` | A3 |
| SI-54 | Technocore içeriği HTML veya tıklanabilir dış link olarak render edilmez (AC-17) | anchor yok, markup yorumlanmaz | `apps/station-web` vitest: `pages.test.tsx` — "never turns a remote URL into a clickable link" | A3 |
| SI-56 | Manifest imza alanı değişirse write gate kapanır | `current` değil → kapı kapalı | `test_manifest_oracle.py::test_the_credentials_are_conditional_not_unconditional_properties`, `test_technocore_readonly.py::test_a_critical_change_makes_the_manifest_not_current` | A3 |
...
| SI-105 | Her iki lane aynı değerlendiriciden geçer | mesaj + note aynı testin iki parametresidir | `test_manifest_oracle.py::test_the_credentials_are_conditional_not_unconditional_properties`, `::test_the_length_bounds_are_numbers_not_strings` (ikisi de `[MESSAGE_BODY_POINTER, NOTE_BODY_POINTER]` ile parametrik) | A3.1 |
...
| ID | Değişmez | Beklenen | Test | Durum |
|---|---|---|---|---|
| SI-55 | Kullanıcı onayı olmadan mesaj/note gönderilemez | onaysız, süresi geçmiş, tekrar kullanılmış ve yabancı oturumdan gelen onay ayrı ayrı reddedilir | `test_compose_flow.py::test_sending_without_an_approval_is_refused`, `::test_an_expired_approval_is_refused`, `::test_a_reused_approval_is_refused`, `::test_an_approval_from_another_session_is_refused`, `::test_the_three_steps_publish_exactly_what_was_approved` | A2 → D |
...
| ID | Değişmez | Beklenen | Test | Durum |
|---|---|---|---|---|
| SI-210 | Modül registry'si **derleme zamanında sabittir**; diskten plugin/import yükleme yolu yoktur ve yasak **dört yazımda birden** aranır | **`station_api` altındaki on sekiz paketin ve on dört gevşek üst-seviye modülün tamamında** — hiçbir paket, hiçbir modül kuralın dışında değil — yasak import, yasak **bare ad** (`runner = __import__`), yasak **attribute** (`builtins.__import__`, `mod.exec`) ve yasak **ad alanı** (`sys.modules`, `getattr(builtins, ...)`) yok; kapsam elle yazılmış bir listeden değil, ağacı **hem dizinler hem de gevşek dosyalar için** yürüyen iki muhafızdan gelir; kuralın tek izni `resources.py` içindeki `importlib.resources` **üyesidir** — paket değil üye: aynı dosyada `import importlib`, `importlib.util` veya `import_module` kırmızı kalır; `compile`'ın yalnız bare adı yasak (`re.compile` serbest); `getattr` adları yalnız `EvidenceField` enum'undan gelir | `test_module_registry.py::test_no_module_is_ever_loaded_from_disk`, `::test_every_package_is_scanned_or_is_a_written_down_registry_exception`, `::test_a_planted_violation_is_reported_from_every_scanned_package`, `::test_every_loose_module_is_scanned_and_every_scanned_module_exists`, `::test_a_planted_violation_is_reported_from_every_scanned_loose_module`, `::test_the_importlib_allowance_is_the_data_reader_and_not_a_loader`, `::test_every_module_allowance_is_used_and_is_scoped`, `::test_the_only_computed_attribute_names_come_from_the_field_enum`, `::test_the_registry_is_a_closed_set_with_unique_identifiers`, `::test_an_unregistered_module_cannot_be_looked_up` | F |
| SI-211 | Kayıt var olan koda **işaret eder**; Proje 0 taşınmaz ve registry paketi sorumluluk üstlenmez | her `owners` girdisi bir dosyaya çözülür; `modules/` dört dosyadır ve `compose`/`evidence`/`vault` import etmez; `planned` kayıt kod sahiplenmez | `test_module_registry.py::test_project_zero_is_represented_and_its_code_was_not_moved`, `::test_no_module_record_moved_code_into_the_registry_package`, `::test_the_registry_satisfies_the_planned_module_contract`, `::test_no_module_is_registered_as_planned_any_more`, `::test_the_planned_module_contract_would_catch_a_record_that_breaks_it` | F |
...
| SI-213 | Görev katmanı **ve Paket H2'nin `agent/` paketi** dördüncü giden yüzey, ikinci kasa/signer veya ikinci gate açmaz | Giden yüzey ve kasa/signer taramalarının açtığı **her** pakette **ve on dört gevşek üst-seviye modülün tamamında** HTTP istemcisi, `socket`, `urllib`, `station_api.vault` ve `station_api.compose` import'u yok. Gevşek modül muaf edilemez — bir modül tek dosyadır, onu muaf etmek kuralın bütün yazımlarını o dosyaya vermek olurdu; onun yerine **adlandırılmış izin** verilir: `app.py` montaj noktası olduğu için beş kasa/composer adını ve iki gözden geçirilmiş istemciyi, `launcher.py` ise yalnız **dinleyen** soketi (INV-02) adıyla taşır, her biri gerekçesiyle ve ekili ihlalle sürülerek. İki kural **ayrı ayrı** kapsanır: bir paket birinin dışında, ötekinin içinde olabilir ve `planner` tam olarak öyledir — giden yüzey muafiyeti var, kasa/signer taraması ise onu kapsıyor. Muafiyetlerin her biri kural adıyla ve gerekçesiyle yazılıdır ve **ekili ihlalle sürülür**: kapsandığı söylenen paket ihlali bildirmek, muaf olduğu söylenen paket bildirmemek zorunda; `tasks.gate.CheckState` **write_gate'inkiyle aynı nesnedir** ve `gate.py` paralel enum tanımlamaz. **H2 ile genişletildi (SI-285):** aynı yasak `station_api/agent` ve `routes/agent.py` üzerinde de uygulanır ve `station_api.vault`'tan yalnız **iki** modül (`windows_acl`, `errors`) birebir allow-list ile serbesttir | `test_module_registry.py::test_the_task_layer_has_no_outbound_surface`, `::test_the_task_layer_reaches_no_vault_and_no_signer`, `::test_every_loose_module_is_scanned_and_every_scanned_module_exists`, `::test_a_planted_violation_is_reported_from_every_scanned_loose_module`, `::test_every_module_allowance_is_used_and_is_scoped`, `::test_the_task_gate_reuses_the_write_gates_check_state`, `test_agent_boundary.py::test_the_agent_package_has_no_outbound_surface`, `::test_the_agent_package_reaches_no_signer_vault_or_credential`, `::test_the_only_vault_imports_are_the_two_that_carry_no_secret`, `::test_the_agent_package_declares_no_second_gate`. **H3 ile yine genişletildi (SI-310):** aynı yasak `station_api/proof` ve `routes/proof.py` üzerinde de uygulanır ve orada **hiçbir vault muafiyeti yoktur** — paket dosya sistemine hiç dokunmadığı için ACL yardımcısına ihtiyacı yok; ayrıca `test_proof_boundary.py::test_the_proof_package_has_no_outbound_surface`, `::test_the_proof_package_reaches_no_signer_vault_or_credential`, `::test_the_proof_package_declares_no_second_gate` | F, H2, H3 |
...
| SI-242 | DB'ye yalnız **göreli yol + zaman + fingerprint** girer; saklanan anahtarı geri gösteren/kopyalayan endpoint **yoktur** ve sütun adlarında `key` yasaktır | beş sütun; yol göreli; yanıt gövdelerinde `api_key`/`key`/`credential`/`token` alanı yok; route kümesi tam **altı** (ADR-0015 probe'u ekledi) ve **hiçbiri okuma yolu değil** — sayı değil, o cümle değişmezdir | `test_opencode_credentials.py::test_only_a_relative_path_a_time_and_a_fingerprint_reach_the_database`, `test_opencode_http.py::test_the_status_document_has_no_field_that_could_hold_a_credential`, `::test_the_surface_offers_exactly_six_routes_and_no_completion_lane`, `test_module_registry.py::test_the_opencode_tables_have_no_secret_shaped_columns` | G |
| SI-243 | Anahtar canary'si **hiçbir yüzeyde** görünmez: başarılı yolun HTTP gövdeleri **ve header'ları**, **reddedilen bir gövdenin 422'si**, OpenAPI, SQLite, zarf, veri dizini, log+exception, frontend bundle. Canary depoda başka yerde yoktur | her yüzeyde yok; canary redaksiyon eşiğinin üzerinde ve benzersiz. **Düzeltme:** ilk yazımda "HTTP gövdeleri ve header'ları" deniyordu ama yalnız **başarılı** store sonrası GET'ler ölçülüyordu; hata yolu kapsam dışıydı ve orada anahtar fiilen yankılanıyordu (bkz. SI-257) | `test_opencode_leakage.py::test_the_credential_is_absent_from_the_envelope_file`, `::test_the_credential_is_absent_from_the_sqlite_database`, `::test_the_credential_is_absent_from_every_http_response_and_its_headers`, `::test_the_credential_is_absent_from_the_openapi_document`, `::test_the_credential_is_absent_from_logs_and_exceptions`, `::test_the_credential_is_absent_from_the_frontend_bundle`, `::test_no_artefact_anywhere_in_the_data_directory_carries_the_credential` (yedi yüzey; yedincisi jokerin **tutmadığı** addı — ADR-0011 §2), `::test_a_rejected_credential_body_is_not_quoted_back_in_the_422`, `::test_the_canary_appears_nowhere_else_in_the_repository`, `::test_the_canary_is_long_enough_for_the_redaction_registry_to_hold_it` | G |
...
| SI-252 | Streaming **yoktur ve yokluğu söylenir**; tool-call **ölçüldü** ve alan ölçülen gerçeği söyler; üç ailenin gövde şeklinin kaynağı beyan edilir | `streaming_supported` `False` **kalır** (akış biçimi ölçülmedi, tahmin edilmez); `tool_calls_supported` ADR-0012'nin ölçümünden sonra `True`'dur ve **hangi ucun** ölçüldüğü `tool_call_provenance` ile telde durur; erteleme cümlesi streaming'i **hâlâ** adlandırır; `shape_provenance` yanıtta | `test_opencode_protocols.py::test_streaming_is_absent_and_says_so_and_tool_calls_were_measured`, `::test_the_shape_provenance_is_stated_rather_than_implied`, `test_opencode_http.py::test_the_protocol_context_states_the_deferral_and_the_measurement` | G, H4 |
| SI-253 | Bağlantı denetimi **gerçek bir istek gönderir ve yalnız basışla**; yeşil rozetin tek üreticisi ölçülü uçtan gelen bir `200`'dür, gerekçeler çoğuldur ve probe'un yanlışladığı gerekçe **düşürülür**. Biçim kontrolüyle başarı üretilmez | **Bu satır ADR-0015 ile yeniden yazıldı.** Eskiden "denetim rozet üretmez, `verified` diye bir durum yoktur" diyordu; o cümle beş paket boyunca doğruydu ve arkasında **hiçbir şey yapmayan bir düğme** vardı (`check_connection` sabit hüküm döndürüyordu, ön yüz aynı `GET /status`'u çağırıyordu, check ucu yoktu). Bugünkü değişmez: `GET /status`, `POST /credential` ve `POST /catalog/refresh` **hiçbir şey göndermez** (her istekte patlayan transport ile ölçülür); yalnız `POST /api/opencode/check` gönderir; sonuç dört durumdan biridir ve hiçbiri diğerine katlanmaz; başarısız bir probe **önceki hükmün yerine geçer**; katalog okumak hâlâ durumu değiştirmez | `test_opencode_probe.py::test_a_saved_key_can_reach_a_verified_verdict_when_the_provider_accepts_it`, `::test_reading_the_status_sends_nothing`, `::test_storing_a_key_does_not_probe_it`, `::test_a_catalog_refresh_is_not_a_probe`, `::test_a_failed_probe_replaces_a_verified_verdict_rather_than_leaving_it`, `::test_a_verified_verdict_drops_the_sentences_the_probe_made_false`, `test_opencode_http.py::test_storing_a_credential_answers_saved_and_not_verified`, `::test_a_fresh_install_reports_not_configured_and_never_verified`, `::test_the_status_read_never_reaches_the_provider`, `test_opencode_catalog.py::test_a_catalog_read_says_nothing_about_the_credential` | G, ADR-0015 |
...
| SI-255 | Görev katmanı **OpenCode'a da** erişemez; yasak liste `station_api.opencode`'un tamamını kapsar | Giden yüzey taramasının açtığı **her** pakette import yok; `planner` bu kuralın tek adlandırılmış muafiyetidir (`opencode.service`'e sorabilir, `opencode.client`'a değil) ve `OUTBOUND_CLIENT_MODULES` beşte kalır | `test_module_registry.py::test_the_task_layer_has_no_outbound_surface` | G |
...
| SI-276 | Üçüncü otorite seviyesi (`community`) tanımlıdır: **yollar seviye 1, içerikleri seviye 3**. `topic` bir onay değildir, `from` `did:key` değilse kendi beyan ettiği takma addır ve uzak içerik **veridir** | `path_authority == 1`, `authority == 3` her mesajda ve her oda kaydında; `did:key` satırı bile "bu mesaj o anahtarla imzalandı" demez; bir karakter fazla/eksik benzeri dizi takma ad sayılır; görünmez ve bidi karakterler süpürülür; yanıtın kendi `untrusted` listesi kısaltılsa bile güven genişlemez — bildirim **telde taşınır** ve geçerli küme iki listenin *birleşimidir*: bir yanıt kümeyi genişletebilir, daraltamaz; servisin kendi ölçümleri çağıranın yazdığı alanlardan **ayrı bir alanda** gelir ve hiçbirinden sıralama veya uygunluk türetilmez | `test_work_scan_snapshot.py::test_room_content_is_level_three_even_though_the_path_is_level_one`, `::test_a_did_key_author_and_a_nickname_get_different_sentences`, `::test_a_lookalike_did_is_treated_as_a_nickname`, `::test_the_caller_written_fields_are_named_in_our_own_module`, `::test_invisible_characters_in_remote_text_are_swept`, `test_work_scan_rooms.py::test_the_service_measurements_and_the_caller_written_fields_are_separate`, `::test_the_replys_own_untrusted_object_is_carried_and_never_relied_on`, `::test_a_reply_that_names_fewer_untrusted_fields_does_not_widen_what_we_trust`, `::test_a_reply_that_names_more_untrusted_fields_widens_it_rather_than_less`, `::test_a_poisoned_topic_is_kept_as_data_and_reaches_no_instruction`, `::test_no_module_in_the_package_reads_a_topic_into_a_derivation`, `::test_the_untrusted_object_is_visible_on_the_wire` | H1 |
| SI-277 | `PUBLIC_ROOM_SCAN` kaynaklı görev **hiçbir görünümde** `OPERATOR_REQUEST` gibi sunulamaz; ayrım **iki bağımsız katmandadır** (farklı kaynak → farklı `source_version_id`, ve farklı başlangıç durumu) | `SUGGESTED` üretilebilir ve **`INITIAL_STATE` `AWAITING_APPROVAL` kalır**. *Paket J güncellemesi:* ilk yazımda "`RUNNING`/`PAUSED` üretilemez kalır" deniyordu; H2'nin yürütücüsü ikisini de üretilebilir yaptı ve `UNPRODUCIBLE_STATES` bugün **boştur** (ADR-0008 §3). Ayrımı taşıyan iki katman değişmedi: farklı `source_version_id` ve farklı başlangıç durumu; iki üretici birbirinin kaynağını reddeder; bayt-birebir aynı içerik iki kaynakta **farklı** `source_version_id` üretir; `suggested` görev ancak kullanıcının geçişiyle onaya gider ve kanıtsız yayıma geçemez | `test_task_states.py::test_the_initial_state_is_not_suggested`, `::test_every_defined_state_is_now_producible_and_nothing_is_left_closed`, `::test_the_refusal_mechanism_still_closes_a_state_when_one_is_closed`, `::test_no_code_path_can_produce_an_unproducible_state`, `test_work_scan_http.py::test_the_two_producers_refuse_each_others_sources`, `::test_the_same_content_gets_a_different_identity_under_the_two_sources`, `::test_suggesting_a_candidate_opens_a_task_in_suggested_and_approves_nothing`, `::test_a_suggested_task_still_has_to_be_approved_by_a_person` | H1 |
...
| SI-294 | Bütçe dolması, araç hatası, kullanıcı iptali, üretilmeyen çıktı ve yeniden başlatma **ayrı ayrı** raporlanır; durdur sonraki çağrıyı engeller ve **geç yanıt yan etki üretmez**; açılışta otomatik devam yoktur | beş ayrı faz ve ayrı cümleler; durdurulmuş çalışmada çalışma alanı **boş**; çağrı sırasında durdurma bayrağı kalkarsa adım `skipped` yazılır ve **yalnızca o çağrı geri alınır**: adım yeni bir dosya oluşturduysa dosya kaldırılır, var olan bir dosyanın üstüne yazdıysa **önceki baytlar geri yüklenir**; `interrupted_runs()` listeler, hiçbir şey devam etmez ve `create_app` sırasında `agent_run` satırı ve aktivite satırı **sıfırdır**; devam yalnız kullanıcının `resume`'udur | `test_agent_runtime.py::test_the_ceiling_ends_a_run_with_its_own_phase_and_audit_event`, `::test_a_tool_failure_is_a_different_ending_from_the_ceiling`, `::test_a_promised_artifact_that_was_not_produced_is_not_a_success`, `::test_a_stop_blocks_the_next_tool_call`, `::test_a_result_arriving_after_a_stop_leaves_no_new_file_behind`, `::test_a_result_arriving_after_a_stop_restores_the_previous_bytes`, `::test_a_run_interrupted_by_a_restart_is_listed_and_never_resumed`, `::test_creating_the_application_starts_no_run` | H2 |
...
### H4 sonrası: aynı kusurun yedinci ve sekizinci örneği

`BUDGET_SCANNED_DIRS` ve `REGISTRY_SCANNED_DIRS` üçer paket okuyordu ve
ADR-0012'nin eklediği iki paketi — `planner` ve `opencode` — hiçbiri
kapsamıyordu. Kusur iddia edilmedi, **ölçüldü**: önce ihlal ekildi ve testin
yeşil kaldığı görüldü, sonra muhafız yazıldı.

| Ekilen ihlal | Eski tarama | Yeni tarama |
|---|---|---|
| `planner/` içine `estimated_budget = 10` ve `run.budget_left` | **yeşil** (55 test, sıfır kırmızı) | **1 kırmızı**: `test_task_evidence.py::test_the_task_layer_opens_no_budget_field` |
| `planner/` ve `opencode/` içine `import importlib` + `loader = importlib.import_module` | **yeşil** (37 test, sıfır kırmızı) | **1 kırmızı**: `test_module_registry.py::test_no_module_is_ever_loaded_from_disk` |

Yeni muhafızların kendisi de mutasyonla sürüldü. İkisi de listeyi değil
**ağacı** yürüyor; ikisinde de listeyi okuyan bir sürüme çevrilince, iki
listede de adı geçmeyen ekili bir paket görünmez oluyor:

| Mutasyon | Ekili paket | Sonuç |
|---|---|---|
| `_packages` ağacı yürüyor (asıl hâli) | `station_api/spend_probe/` | **kırmızı** — `test_every_package_is_scanned_or_is_a_written_down_budget_exception` ve `::..._registry_exception` |
| `_packages` iki listeyi okuyor (mutasyon) | aynı paket, yerinde | **yeşil** — muhafız kör |

Muafiyetlerin gerekçeleri de düzyazıda bırakılmadı:
`test_the_ceiling_is_named_by_exactly_the_modules_written_down_here`,
`::test_the_reviewed_outbound_modules_are_still_five` ve
`::test_the_signer_is_named_by_exactly_the_modules_written_down_here`
gerekçelerin dayandığı olguyu `TaskRecord` sabitlemesinin şekliyle sabitler.
İlki daha ilk koşuda bir gerekçeyi yanlış çıkardı: `workscan` tavanı hiç
import etmiyordu, sözü düzeltildi.
...
yazıldı**: tarama birimi *paket*ti, yani `station_api` altında doğrudan duran
on dört gevşek `.py` dosyası (`app.py`, `schemas.py`, `resources.py`, …) üç
registry kuralının ve bütçe kuralının **dışındaydı**. Yorumdaki bir not
muhafız değildir, ve kusur burada da iddia edilmedi, **ölçüldü**: dört gevşek
modül ekildi ve iki dosyanın 175 testinin tamamı yeşil kaldı.

| Ekilen gevşek modül | Eski tarama | Yeni tarama |
|---|---|---|
| `import importlib` + `loader = importlib.import_module` | **yeşil** (175 test, sıfır kırmızı) | **kırmızı**: `test_module_registry.py::test_no_module_is_ever_loaded_from_disk` |
| `import requests` | **yeşil** (aynı koşu) | **kırmızı**: `::test_the_task_layer_has_no_outbound_surface` |
| `from station_api.vault.service import VaultService` | **yeşil** (aynı koşu) | **kırmızı**: `::test_the_task_layer_reaches_no_vault_and_no_signer` |
| `estimated_budget = 10` + `run.budget_left` | **yeşil** (aynı koşu) | **kırmızı**: `test_task_evidence.py::test_the_task_layer_opens_no_budget_field` |

Dördü de tuple'a eklenmeden **önce** yeni yürüyüş muhafızı iki kırmızı verir
(`::test_every_loose_module_is_scanned_and_every_scanned_module_exists`,
her iki dosyada); tuple'a eklendiğinde kuralların kendisi yukarıdaki dört
kırmızıyı verir. Yani hem "listeyi genişletmeyi unutan" hem de "listeyi
genişletip ihlali bırakan" yol kapalıdır.

Yeni muhafız da mutasyonla sürüldü, tam olarak yedinci/sekizinci örnekteki
gibi: keşif **glob** yerine elle yazılmış dosya adı listesini okuyacak biçimde
çevrildi.

| Mutasyon | Ekili modül | Sonuç |
|---|---|---|
| `_loose_modules` dizini `glob("*.py")` ile yürüyor (asıl hâli) | dört gevşek modül, yerinde | **2 kırmızı** — iki dosyanın yürüyüş muhafızı |
| `_loose_modules` `REGISTRY_SCANNED_MODULES` / `BUDGET_SCANNED_MODULES` okuyor (mutasyon) | aynı dört modül, yerinde | **yeşil** (236 test) — muhafız kör |

`resources.py` kararı dürüstçe verildi ve kuralı herkes için gevşetmedi.
`importlib.resources` ADR-017'nin yasakladığı plugin yükleme yolu **değildir**:
paketlenmiş veri dosyasını okur, keyfi kod yükleyemez. Bu yüzden muafiyet
modüle değil **üyeye** yazıldı — `resources.py` içinde yalnız
`import importlib.resources`. `dynamic-loading` kuralının paket düzeyinde hâlâ
**sıfır** muafiyeti vardır ve ağaçtaki tek adlandırılmış izin budur;
`test_the_importlib_allowance_is_the_data_reader_and_not_a_loader` bunu ekili
`importlib.import_module`, düz `import importlib`, `from importlib import
import_module` ve `spec_from_file_location` yazımlarıyla sürer — dördü de
kırmızı.
...
## 9m. Paket H4 değişmezleri — model plan yolu ve kabul koşulları (uygulandı)

Bağlayıcı karar: ADR-0012. Bu paket **hiçbir güvenlik değişmezini
gevşetmez**; iki *olguyu* değiştirir ve o olgulara dayanan cümleleri düzeltir.
Neyin neden değiştiği [`model-planning.md`](model-planning.md) §0'da yazılıdır.

| ID | Değişmez | Beklenen | Test | Durum |
|---|---|---|---|---|
| SI-330 | Tool-call biçimi **ölçüldü, uydurulmadı**; istek **registry'nin tamamını** sunar ve **çağrı zorlamaz** | istek gövdesindeki `tools` adları `TOOLS`'un adlarıyla **birebir aynı** kümedir, `tool_choice` `auto`, `stream` `false`; hiçbir parametre şeması `path`/`url`/`command`/`cwd` adı taşımaz ve hepsi `additionalProperties: false`; `chat/completions` dışında bir protokol ailesi için istek **kurulmaz** | `test_model_planner.py::test_the_request_offers_the_whole_registry_and_forces_no_call`, `::test_a_tool_message_names_the_call_it_answers` | H4 |
| SI-331 | `reasoning_content` **saklanmaz, gösterilmez, loglanmaz** — koruma tip düzeyindedir, gösterim yolunda ise deny-list'tir | ölçülen yanıttaki alan adı `DISCARDED_MESSAGE_FIELDS` içindedir ve ayrıştırıcıdan çıkan değerde işaretçi **hiçbir alanda** yoktur; bir model turundan sonra hiçbir aktivite satırının `detail`'i işaretçiyi taşımaz; HTTP yanıtının **tamamında** geçmez; `error` üyesi taşıyan bir `200`'ün kullanıcıya gösterilen alıntısında işaretçi yoktur ama sağlayıcının hata metni **vardır**; ayrıştırılamayan ve bu alanlardan birini anan bir gövde hiç alıntılanmaz | `test_model_planner.py::test_the_reasoning_field_is_dropped_rather_than_carried`, `::test_a_quoted_provider_error_body_carries_no_reasoning`, `::test_an_unreadable_body_that_names_a_reasoning_field_is_not_quoted`, `::test_no_timeline_row_from_a_model_turn_carries_the_reasoning`, `test_agent_boundary.py::test_no_agent_table_can_hold_a_model_reasoning_trace` | H4 |
| SI-332 | Model çıktısı **doğrudan yürütülmez**: her çağrı kapalı registry'den geçer ve **tek bir kayıtsız çağrı tüm öneriyi düşürür** | kabuk adı, kayıtsız araç, dosya adında `..` ve bildirilmemiş parametre dört ayrı **ret**tir; hiçbirinde çalışma kaydedilmez, çalışma alanı boş kalır ve görev **yerinde durur**; geçerli+geçersiz karışık öneride de hiçbir şey kaydedilmez; ret `permission_denied` olarak **hiç budanmayan zincire** girer | `test_model_planner.py::test_a_proposal_outside_the_registry_is_refused_whole`, `::test_one_bad_call_drops_the_whole_proposal`, `::test_a_refused_proposal_reaches_the_audit_chain` | H4 |
| SI-333 | Model **kendi planına onay veremez**: öneri `planned` fazında kalır ve başlatmayı bir kişi yapar | planner ağacında ve rotasında `start_run`/`resume_run`/`request_stop` adları **hiç geçmez** (çağrı, öznitelik ve çıplak ad olarak); tarama ekili üç yazımda kırmızıdır; öneriden sonra çalışma `planned`, görev `awaiting_approval`, çalışma alanı **boş**; aynı çalışma ancak `start_run` ile koşar | `test_planner_boundary.py::test_the_planner_cannot_start_a_run`, `::test_the_run_starter_scan_would_see_a_planted_call`, `test_model_planner.py::test_a_proposal_becomes_a_recorded_plan_and_nothing_runs`, `::test_the_recorded_plan_runs_only_when_a_person_starts_it` | H4 |
| SI-334 | Model yolu **kendi giden yüzeyini açmaz**: `OUTBOUND_CLIENT_MODULES` **beşte** kalır | planner ağacı `httpx`, `socket`, `ssl`, dört istemci modülü ve **`opencode.client`** dâhil hiçbirini import etmez; `station_api.opencode` altından yalnız `service`, `planner` ve `errors` görünür ve muafiyet **tam liste**dir; kimlik bilgisi zarfına (`credential_store`) erişilmez; hiçbir şey zamanlanmaz | `test_planner_boundary.py::test_the_planner_opens_no_outbound_surface_of_its_own`, `::test_the_outbound_exemption_is_an_exact_list_and_is_used`, `::test_the_planner_reaches_no_signer_vault_or_credential_store`, `::test_nothing_here_schedules_a_second_turn`, `test_write_gate.py::test_httpx_is_imported_only_by_the_reviewed_clients` | H4 |
| SI-335 | Tavan **bizim sayabildiğimiz** birimdedir; `usage` ve `cost` **kaydedilir, tavan olarak okunmaz** | birimler dört (`tool_call_count`, `model_call_count`, `wall_clock_seconds`, `concurrency`) ve `refused_units` hâlâ `("token","currency")`; model çağrısı tavanı **kendi reddini** verir ve tavana ulaşan tur **hiçbir istek göndermez**; sağlayıcı sayı göndermediğinde `unknown`, sıfır **uydurulmaz**; `cost` **dize** olarak kalır | `test_agent_budget.py::test_the_units_are_the_ones_this_build_can_measure`, `::test_there_is_no_token_and_no_currency_unit`, `::test_the_model_call_ceiling_refuses_by_name`, `::test_the_call_ceiling_is_checked_before_the_clock`, `test_model_planner.py::test_a_session_stops_at_the_model_call_ceiling`, `::test_usage_and_cost_are_recorded_as_sent_and_never_invented` | H4 |
| SI-336 | Döngü **kapanır** ve sağlayıcı hatası **cevap sayılmaz** | araç sonucu `role:"tool"` mesajıyla ve **koşucunun kendi cümlesiyle** döner, dosyanın baytları **dönmez**; `finish_reason` `tool_calls` olmaktan çıkınca oturum biter ve boş `tool_calls` dizisi **döngü sebebi değildir**; 401/403/429/500 ve 200-içi `error` üyesi plan **kaydetmez** ve metered çağrı **tam bir kez** denenir | `test_model_planner.py::test_the_loop_feeds_results_back_and_ends_when_the_model_stops`, `::test_a_finish_reason_of_tool_calls_with_no_calls_does_not_loop`, `::test_a_provider_failure_records_no_plan`, `::test_a_200_carrying_an_error_member_is_not_an_answer` | H4 |
| SI-337 | Model isteği **gövdeden yönlendirilemez**: model, sistem istemi ve araç listesi istekle **değiştirilemez** | `model`, `system_prompt`, `tools` ve `tool_choice` alanlarını taşıyan gövdeler **422**'dir ve reddedilen gövde sağlayıcıya **ulaşmaz**; rota CSRF ister; yanıt `no-store`'dur ve `model_can_start_a_run` `Literal[False]` taşır | `test_model_planner.py::test_the_route_body_cannot_choose_a_model_or_a_prompt`, `::test_the_route_needs_a_session_and_a_csrf_token`, `::test_the_route_records_a_plan_and_reports_that_it_started_nothing` | H4 |
| SI-338 | Kabul koşulu kümesi **yedinci kapalı registry**dir: derleme zamanı tuple, tipli parametreler, **serbest metin koşulmaz** | kayıt beş koşuldur ve oracle **elle yazılıdır**; kayıtsız ad `ToolRegistryError(reason="acceptance_condition_unknown")` ile **gösterilebilir ret** alır (`run_pytest` dâhil); `path`/`url` parametresi yoktur, `..` taşıyan ad reddedilir; registry çalışma zamanında genişletilemez ve kayıt **dondurulmuştur** | `test_agent_acceptance.py::test_the_registry_is_exactly_the_conditions_this_release_has`, `::test_an_unregistered_condition_gets_a_shown_refusal`, `::test_no_condition_parameter_can_carry_an_address`, `::test_an_argument_of_the_wrong_type_is_refused_while_planning`, `::test_the_registry_cannot_be_extended_at_runtime` | H4 |
| SI-339 | Kabul koşulu **iki yönde de** sürülür ve **koşulsuz plan geçmiş sayılmaz** | beş koşulun her biri hem sağlanan hem sağlanmayan bir argümanla sürülür; olmayan dosya **hata değil, sağlanmamış koşuldur**; koşulsuz küme `not_implemented`'tır ve **asla** `passed` değildir (`all([])` boşluğu kapalı) | `test_agent_acceptance.py::test_a_condition_that_holds_is_not_the_same_as_a_file_existing`, `::test_a_condition_naming_a_missing_file_is_unsatisfied_rather_than_an_error`, `::test_no_condition_at_all_is_not_a_pass` | H4 |
| SI-340 | Kabul koşulları **plan özetinin içindedir** ve onaydan sonra gevşetilemez | yalnız koşulları farklı üç plan **üç farklı** `plan_sha256` üretir; satırdaki `acceptance_json` elle değiştirildiğinde `start_run` `plan_changed` ile reddeder; koşul sayısı tavanlıdır | `test_agent_acceptance.py::test_the_conditions_are_inside_the_plan_digest`, `::test_a_condition_edited_after_approval_refuses_the_run`, `::test_a_plan_cannot_carry_more_conditions_than_the_bound` | H4 |
...
| SI-345 | Model çağrısı tavanı **göreve yazılır ve kalıcıdır**: `forget` de, yeniden başlatma da onu geri vermez; sayaç **yalnız artar** ve **sıfırlama yoktur** | ADR-0012 §3'ün seçtiği birim tek harcama kontrolüdür, o yüzden sayaç bellekteki oturum nesnesinde değil `model_call_ledger` satırındadır (migration `0011`, yalnız ekleme, `task_id` **birincil anahtar** — yani görev başına). Tavan dolduktan sonra üç kez "unut + tur iste" ölçülü uca **sıfır** istek gönderir ve sayan taraf transport recorder'dır; aynı motora kurulan **ikinci bir servis** — yeniden başlatmanın kendisi — de sıfır istek gönderir; ikinci bir görev sıfırdan başlar. `station_api` ağacının **tamamında** `model_calls_used`'a tek yazı vardır (`record_call`) ve o yazı bir **toplamadır**; satırı **silmek** de bir düşürmedir, o yüzden sayaç modülü hiçbir şey silmez ve `ModelCallLedger` **tam iki modülde** anılır — bu ikinci kural mutasyonla bulundu, yalnız niteliği izleyen tarama silme yazımını görmüyordu | `test_model_planner.py::test_forgetting_a_session_does_not_forget_the_spend`, `::test_forget_cannot_be_clicked_for_a_second_ceiling`, `::test_the_ceiling_survives_a_restart_of_the_process`, `::test_the_ceiling_is_counted_per_task_and_not_across_them`, `::test_nothing_lowers_the_model_call_counter`, `::test_the_counter_write_scan_would_see_a_planted_reset`, `::test_the_ledger_row_is_never_deleted_and_reaches_only_two_modules`, `::test_the_only_counter_write_is_an_increment`, `test_agent_boundary.py::test_migration_0011_added_one_table_and_touched_nothing_else` | H4, ADR-0013 || SI-346 | Ekranın **okunan cümlesi** de denetlenir: bir sonucun rozeti, sonucun kendisinden farklı bir şey söyleyemez ve sağlayıcının muhakemesi hiçbir alan adıyla ekrana ulaşamaz | Model turunun yedi bitişinin **rozeti** sürülür: yalnız `planned` "plan kaydedildi" diyebilir ve hiçbiri `ok` tonunu (✓ / `durum: iyi`) taşımaz; test hükmünün üç değeri de kendi cümlesi ve kendi tonuyla pinlenir (`Gecti`/`iyi`, `Kaldi`/`sorunlu`, `Uygulanmadi`/`etkin degil`) ve `passed` dışındaki ikisi `Gecti` ve ✓ taşıyamaz; yanıtta `reasoning_content`, `reasoning` ve `thinking` adlarıyla gelen bir kanarya **belgenin hiçbir yerinde** görünmez; araç çağrısı cümlesi `tool_calls_supported: false` iken "ölçülmüş değildir" der; keşif günlüğünün yazma reddi **iki farklı yük** ile sürülür, böylece istemciye yazılmış bir sabit ikisinden birini geçemez; duyurulan oda adının "bunu bir yabancı yazdı" başlığı, nitelediği denetimin bulunduğu satırın içindedir | `TasksPanel.test.tsx::does not let the refused ending wear the recorded plan's words or its tone`, `TasksPanel.test.tsx::words and tones the one ending that recorded a plan as the recorded plan`, `TasksPanel.test.tsx::renders the not_implemented verdict as its own sentence and its own tone`, `TasksPanel.test.tsx::renders nothing from a reasoning field the provider sent`, `TasksPanel.test.tsx::says the tool-call format was measured only when the wire says it was`, `WorkScanPanel.test.tsx::shows the payload's own write refusal rather than one of its own`, `WorkScanPanel.test.tsx::labels an announced room name as something a stranger wrote` | H4, ADR-0012, ADR-0013 |
| SI-347 | Denetim probe'u **kimlik bilgisi başına** sayılır ve tavanlanır; `forget` + aynı anahtarı yeniden kaydetmek de yeniden başlatma da tavanı geri vermez, ve **sıfırlama yoktur** | Ölçülü uca giden sayılmayan bir çağrı, ADR-0013'ün kurduğu tavanın yanındaki açık kapıdır. Sayaç `opencode_probe_ledger` satırındadır (migration `0012`, yalnız ekleme, **parmak izi birincil anahtar ve FK yok** — kimlik bilgisi satırına yazılsaydı `forget` iki basışta tavanı geri verirdi); tavan `budget.py::MAX_CONNECTION_PROBES = 8`'dir ve birim `BUDGET_UNITS`'in zaten adlandırdığı model çağrısıdır. Tavandan sonra üç basış ölçülü uca **sıfır** istek gönderir ve sayan taraf transport recorder'dır; ikinci bir servis (yeniden başlatmanın kendisi) de sıfır gönderir; **başka bir anahtar** kendi tavanıyla başlar. `station_api` ağacının tamamında `probes_used`'a tek yazı vardır ve o bir toplamadır; satırı silen hiçbir kod yoktur ve `OpenCodeProbeLedger` **tam iki modülde** anılır. **Kaybedilen yanıt sayılır** — `OpenCodeLostResponseError` "bu istek ücretlenmiş olabilir" demektir | `test_opencode_probe.py::test_the_ceiling_stops_the_button_and_says_where_you_stand`, `::test_forgetting_and_re_saving_the_same_key_does_not_hand_back_the_ceiling`, `::test_the_ceiling_survives_a_restart_of_the_process`, `::test_a_different_key_starts_with_its_own_ceiling`, `::test_a_lost_answer_is_inconclusive_and_is_still_counted`, `::test_nothing_lowers_the_connection_probe_counter`, `::test_the_probe_counter_scan_would_see_a_planted_reset`, `::test_the_probe_ledger_row_is_never_deleted_and_reaches_only_two_modules`, `::test_migration_0012_added_one_table_and_touched_nothing_else` | H4, ADR-0015 |
| SI-348 | Probe kimlik bilgisini sağlayıcıya **bilerek** gönderir ve başka hiçbir yere ulaştırmaz: hükme, gerekçeye, saklanan satıra, veritabanı dosyasına, loga veya hata alıntısına değil | Kanarya, bir upstream'in koyacağı yere ekilir — kendisini taşıyan isteğin **401 hata gövdesine** — ve sonra bu özelliğin yazdığı her yerde aranır; alıntı `client.py::_excerpt`'te, redaksiyon penceresinin **içinde** hesaplandığı için `<redacted>` görünür (hiçbir şey göstermeyen bir tanı, birinin sildiği tanıdır) ve `DISCARDED_MESSAGE_FIELDS` aynı fonksiyonda uygulanır. Probe isteği tek bir `Authorization` başlığı taşır ve **araç registry'si taşımaz**; istek gövdesinde çağıranın yazdığı hiçbir alan yoktur | `test_opencode_probe.py::test_a_provider_that_echoes_the_credential_leaks_it_nowhere`, `::test_the_probe_request_carries_the_credential_and_no_tool_registry`, `test_opencode_http.py::test_the_check_route_takes_no_body_that_could_steer_it`, `::test_no_response_in_the_chain_carries_the_stored_credential` | G, H4, ADR-0015 |

### Mutasyon kaydı (SI-346) — "ekran yalan söylerse fark eden olur mu?"

Yeni yüzeylerde **26 mutasyon** sürüldü; her biri yeniden derlendi ve hem
Vitest hem ilgili Playwright dosyası koşuldu. Yirmisi en az bir katmanda
kırmızıya döndü. **Altısı hiçbir katmanda görülmedi**, ve altısı da aynı
şekle sahipti: iddiayı taşıyan asıl cümle test edilmiyordu, yalnız yanındaki
makine değeri test ediliyordu.

| Mutasyon | Ne yalan söyledi | Kim yakaladı |
|---|---|---|
| `refused` bitişine `planned` etiketi + `ok` tonu | kayıtsız araç önerisi reddi, **yeşil tikli "plan kaydedildi"** olarak | hiç kimse |
| `not_implemented` hükmüne "Gecti" cümlesi + `ok` tonu | koşulsuz plan, **geçmiş test** olarak | hiç kimse |
| yanıttaki `reasoning_content` ekrana basıldı | ADR-0012 §1 | hiç kimse (hiçbir fixture böyle bir alan göndermiyordu) |
| araç çağrısı cümlesi koşulsuz "ölçülmüştür" | ölçülmemiş sözleşme, ölçülmüş gibi | hiç kimse |
| keşif günlüğünün "bunu bir yabancı yazdı" başlığı silindi | çıplak bir onay kutusu, yabancının seçtiği adla | hiç kimse |
| yazma reddi cümlesi istemciye sabitlendi | okunmuş cümle, yazılmış cümle olarak | hiç kimse |

Kalan yirmisinden ikisi ayrıca bir **katman haritası** verdi: tam 64-hex özet
yalnız **e2e** tarafından (`proof.spec.ts`, gerçek DOM taraması), `dangerouslySetInnerHTML`
ise yalnız **kaynak taraması** tarafından yakalanıyor. İkisi de bilinçli
yerleşimdir; ikisi de tek katmandır.

Yeni testler **yalnız bileşen katmanına** kondu: altı iddianın altısı da
render kuralıdır (kapalı bir haritadan çıkan etiket ve ton, bir alanın DOM'a
ulaşıp ulaşmadığı, bir cümlenin yükten mi sabitten mi geldiği) ve bunların
hiçbiri gerçek tarayıcı, gerçek backend veya gerçek CSP istemez. Her yeni
test kendi mutasyonu yeniden uygulanarak **kırmızı** görüldü, mutasyon geri
alındıktan sonra **yeşil**.


### SI-252, SI-286 ve SI-225 neden **güncellendi**, silinmedi

**SI-252** ("streaming ve tool-call yoktur ve yokluğu söylenir") yarı yarıya
doğru kaldı. Streaming **hâlâ yoktur** ve `streaming_supported` `False`'tur:
akış biçimi ölçülmedi. Tool-call biçimi **ölçüldü**, bu yüzden
`tool_calls_supported` artık ölçülen gerçeği söyler ve erteleme cümlesi
ikiye ayrıldı. Alanın kendisi silinmedi: bir sürüm önce `false` gören bir
istemciye alanın kaybolması değil, **değerin değiştiği** söylenir.

**SI-286** ("araç registry'si altıncı kapalı registry") aynen durur.
Değişen tek şey, o registry'nin şemasının artık bir **tüketicisi** olmasıdır:
`json_schema()` Station'ın kendi şemasını üretir, sağlayıcıya özgü zarfı
adaptör kurar. `OUTBOUND_CLIENT_MODULES` **beşte** kaldı.

**SI-225** ("görev katmanında bütçe alanı yoktur") harfiyen doğru kaldı:
dördüncü birim de `agent/budget.py`'dedir, `tasks/` ve `modules/` ağaçlarında
bütçe biçimli bir tanımlayıcı yoktur ve `budget_available` hâlâ
`Literal[False]`'tur.

---

## 9n. ADR-0014 değişmezleri — oda satırlarını bir model okur (uygulandı)
...
Ayrıntı: [`work-scan.md`](work-scan.md) §3.

Bu bölüm bu depodaki **en riskli** değişikliğin değişmezleridir: yabancıların
yazdığı metin artık bir modele gösteriliyor. Dokuz satırın sekizi ADR-0014'ün
saydığı sekiz özelliktir; dokuzuncusu kullanıcının ölçtüğü kusurdur.

| # | Değişmez | Nasıl sürülüyor | Test | Aşama |
|---|---|---|---|---|
| SI-349 | Oda metni **veridir, talimat değildir**: satırın ne yazdığı ne istenen isteği ne de cevabın işlenişini değiştirir | Beş ayrı enjeksiyon şekli ekilir — düz "önceki talimatlarını yok say", sahte `[system]` bloğu, telde geçerli görünen **sahte bir `tool_calls` dizisi**, kapsayıcıyı kapatıp yeni bir kural bölümü açmayı deneyen satır, ve "kullanıcı bunu önceden onayladı" iddiası. Her birinde istek **aynı istektir**: kurallar `system` turundadır ve ekilen metin orada değildir, mesaj sayısı **ikidir** (oda metninden `assistant` turu üretilmez), sunulan araçlar dört kayıtlı araçtır, ve ekilen satır — kendi satırsonları süpürüldüğü için — bloğun **tek** numaralı satırıdır. Sahte araç çağrısı istekte metin olarak görünür, araç listesinde görünmez; sağlayıcı o adı gerçekten döndürse tur **bütünüyle** reddedilir | `test_work_scan_model_reading.py::test_a_planted_instruction_changes_neither_the_ask_nor_the_answer`, `::test_a_line_that_forges_a_tool_call_cannot_reach_the_registry` | ADR-0014 |
| SI-350 | Model **sınıflandırır, eylemez**: bir oda, adres, dosya, araç veya alıcı adlandıramaz ve hiçbir şeyi başlatamaz | Dört okuma aracının **her birinin** şeması tek bir `lines` parametresi bildirir, `additionalProperties: false` taşır ve on dört adreslenebilir parametre adının hiçbirini içermez; `lines` dışında argüman taşıyan bir çağrı turu **bütünüyle** düşürür. `station_api/workreader` ağacında `start_run`, `resume_run`, `request_stop`, `plan_run`, `suggest_task`, `open_task`, `write_text`, `ensure_workspace`, `record_evidence` ve `transition` adları — çağrı, öznitelik ve çıplak ad olarak — **hiç geçmez**, ve tarama ekili üç yazımda kırmızıdır; ağaçta dosya sistemi ve veritabanı importu yoktur, `.state` ataması yoktur | `test_work_scan_model_reading.py::test_the_reading_registry_names_nothing_addressable`, `::test_an_extra_argument_drops_the_turn`, `::test_the_reading_lane_cannot_start_a_run_or_open_a_task`, `test_work_reader_boundary.py::test_the_reader_cannot_act_on_anything`, `::test_the_action_scan_would_see_a_planted_call`, `::test_the_reader_writes_no_state_and_touches_no_disk` | ADR-0014 |
| SI-351 | Cevap **kapalı bir registry**'den döner: ad aranır, argüman tiplenir, serbest metin ayrıştırılmaz | Kayıtsız bir ad turu bütünüyle düşürür ve **turdaki her satır** gerekçesiyle listelenir; ad araması **birebir**dir (baştaki/sondaki boşluk, büyük harf ve önek eşleşmez); `lines` argümanı yedi ayrı bozuk yazımda ve **turun göndermediği bir numarada** reddedilir. Sağlayıcı düzyazıyla "satır 1 bir help_wanted" dediğinde **hiçbir** verdict üretilmez, ve `station_api/workreader` ağacında `proposal.text`/`proposal.content` özniteliğine yapılmış **tek bir erişim** yoktur | `test_work_scan_model_reading.py::test_an_unregistered_tool_name_drops_the_whole_turn`, `::test_the_name_lookup_is_exact_and_the_arguments_are_typed`, `::test_free_text_in_the_answer_is_not_a_parsing_surface`, `::test_the_reading_lane_reads_no_free_text_field` | ADR-0014 |
| SI-352 | Modelin görüşü aday üretimine bir **girdi**dir, mevcut kapıların etrafından dolaşma yolu değil | Yasak kaydı **iki kez** çalışır: satır modele gönderilmeden önce (o satır sağlayıcıya hiç gitmez) ve verdict okunmadan önce. İkincisi asıl kapıdır ve en sert biçimiyle sürülür: cüzdan satırına **`help_wanted` verdict'i verilir** ve sonuç yine tek bir `wallet_or_payment` reddidir, sıfır aday. `DENIED_ROOMS` okuma yolunda aynen geçerlidir ve reddedilen odada okuyucu **hiç çağrılmaz** (giden istek sayısı sıfır, okuyucunun gördüğü satır sayısı sıfır). Aday kimliği `(room, seq)`'ten, her cümlesi ham alandan veya sabit şablondan gelmeye devam eder | `test_work_scan_model_reading.py::test_a_verdict_about_prohibited_work_produces_a_refusal_not_a_candidate`, `::test_a_prohibited_line_is_never_shown_to_the_model`, `::test_the_scan_still_applies_the_room_policy_with_a_reader`, `::test_a_verdict_cannot_invent_an_identity_or_a_sentence`, `test_work_scan_candidates.py::test_a_prohibited_line_produces_no_candidate_even_when_it_matches_a_signal` | ADR-0014 |
| SI-353 | Harcama **sınırlı ve sayılır**; çok satırlı bir oda çok tur satın alamaz; maliyet **harcanmadan önce** yazılıdır | Tavan `budget.CEILING.max_model_calls` ve kontrol planlama yolunun aynı saf `budget.check` fonksiyonudur. Tavanın üstünde parti verildiğinde ölçülü uca giden istek sayısı — **transport recorder'dan** sayılır — tam tavan kadardır ve kalan satırlar `reading_ceiling` gerekçesiyle listelenir. Satır sayısını **on katına** çıkarmak tur sayısını on katına çıkarmaz. Bir taramanın bütün odaları **tek** sayacı paylaşır (ikinci oda birinciden devam eder). `station_api` ağacının tamamında `_scan_model_calls_used` alanına **tek** yazı vardır ve o bir **toplamadır**; `__init__` bile yazmaz, çünkü alan bir dataclass alanıdır. Cevap gelmeyen tur **sayılmaz**. Maliyet cümlesi her `status` okumasında, sonuç olmadan da gövdededir ve iki sayıyı da taşır | `test_work_scan_model_reading.py::test_a_scan_stops_at_the_model_call_ceiling`, `::test_many_lines_in_one_room_do_not_buy_more_turns`, `::test_one_scan_is_one_ceiling_across_its_rooms`, `::test_nothing_lowers_the_scan_model_call_counter`, `::test_the_scan_counter_write_scan_would_see_a_planted_reset`, `::test_the_cost_is_stated_before_it_is_spent`, `::test_a_turn_that_never_answered_is_not_counted`, `WorkScanPanel.test.tsx::states what a scan will cost before the button is pressed` | ADR-0014 |
| SI-354 | Hiçbir gizli materyal, DID veya anahtar **prompt'a girmez** | Gönderilen gövdenin **baytları** on gizli-şekilli parçaya karşı taranır (`seed`, `mnemonic`, `private_key`, `passphrase`, `did:key:`, `recovery`, `tcrec` ve sentetik kimlik bilgisinin kendisi dâhil) ve gövdede `Authorization` diye bir anahtar yoktur. Daha güçlü ikinci iddia: gövdenin mesajları `build_messages`'ın ürettiğiyle **birebir eşittir**, yani araya sızmış bir değer bu eşitliği bozar | `test_work_scan_model_reading.py::test_no_secret_or_identity_shaped_value_reaches_the_request`, `::test_the_request_is_the_two_constants_plus_the_lines` | ADR-0014 |
| SI-355 | Modelin muhakemesi **saklanmaz ve gösterilmez** (ADR-0012 §1, ikinci lane) | Sağlayıcı yanıtı ölçülen `reasoning_content` alanını taşır; okuma turundan çıkan `RoomReading`'in ve ondan üretilen `DerivationResult`'ın **tamamında** ne işaretçi ne alan adı geçer. Koruma bir silme değil **tip**tir: `RoomReading`, `LineVerdict` ve `ReadingRefusal`'ın hiçbir alanı `reasoning`/`thought`/`content`/`completion`/`payload`/`prompt` adını taşımaz | `test_work_scan_model_reading.py::test_the_reasoning_field_reaches_no_verdict_no_refusal_and_no_candidate`, `::test_no_type_on_the_return_path_can_hold_a_reasoning_trace` | ADR-0014 |
| SI-356 | Sağlayıcı kullanıcı hakkında bir **bilgi kaynağına** dönüşmez; giden yüzey **beşte** kalır | Okuma lane'ine verilen `ReadableLine` **tam iki alan** taşır (`seq`, `text`): oda adı, yazar ve zaman damgası ona hiç verilmez, dolayısıyla dışarıda bırakılacak bir değer yoktur. Uçtan uca sürülür: bütün bir oda taranır ve **oda adı** giden gövdede geçmez; `did:key` şekilli yazar ve zaman damgası da geçmez; bir satır giden yolda sınırlanır ama adaydaki alıntı **tam**dır. `station_api/workreader` ağacı `httpx`, `socket`, `ssl`, dört istemci modülü ve **`opencode.client`** dâhil hiçbirini import etmez; `station_api.opencode` altından yalnız `service`, `planner` ve `errors` görünür ve muafiyet **tam liste**dir; `station_api/workscan` hâlâ ne `opencode` ne `workreader` import eder | `test_work_scan_model_reading.py::test_the_room_name_never_reaches_the_provider`, `::test_the_author_and_the_timestamp_do_not_travel`, `::test_one_line_is_bounded_on_the_way_out`, `test_work_reader_boundary.py::test_the_reader_opens_no_outbound_surface_of_its_own`, `::test_the_outbound_exemption_is_an_exact_list_and_is_used`, `::test_the_reader_reaches_no_signer_vault_or_credential_store`, `::test_the_scan_package_still_reaches_no_model` | ADR-0014 |
| SI-357 | Kalıp listesi **kalktı**, şablonlar kaldı, ve ekran artık "anlamsal çıkarım yoktur" demiyor | Kullanıcının ölçtüğü kusur bir test: sıradan Türkçeyle yazılmış gerçek bir iş satırı — yirmi dokuz markerın hiçbirini taşımayan — artık **bir aday üretir**. `Signal` tipinde `markers` alanı yoktur ve `candidates.py`'nin sözdizim ağacında `markers=` anahtar argümanı **hiç geçmez**; dört şeklin adı ve yedi şablon alanının **hepsi** yerinde durur ve her şekil için tam bir okuma aracı vardır. Dürüstlük cümlesi "anlamsal cikarim yoktur" ve "kalip eslesmesiyle cikarir" ifadelerini **taşımaz**, "dil modeline" ve "yanilabilir" ifadelerini taşır; ekranın başlığı da değişti ve eski cümle DOM'da **yoktur**. Okuyucusu olmayan bir yapı boş liste değil, satır başına `model_unavailable` reddi verir | `test_work_scan_model_reading.py::test_a_room_of_ordinary_language_now_produces_a_candidate`, `::test_the_deleted_marker_list_is_gone_rather_than_shortened`, `::test_the_four_shapes_and_all_of_their_wording_survived`, `::test_the_screen_no_longer_promises_that_nothing_is_inferred`, `::test_a_build_with_no_reader_refuses_every_line_by_name`, `test_work_scan_language.py::test_the_honesty_sentence_is_not_itself_a_forbidden_claim`, `WorkScanPanel.test.tsx::no longer promises that nothing is inferred` | ADR-0014 |

### Neden SI-276 ve SI-278 **güncellenmedi**

SI-276 "aday üretimi deterministiktir" demiyordu; **sekiz öğenin yapısal olarak
zorunlu** olduğunu söylüyordu ve bu hâlâ doğrudur. Aynı şekilde alıntı, kaynak
referansı, izin/risk alanları ve `(room, seq)` kimliği hakkındaki satırların
hiçbiri tanıma yöntemine dayanmıyordu. ADR-0014 **yalnız eşleşmeyi** kaldırdı;
bu yüzden bu bölüm eski satırları düzeltmez, yanlarına dokuz satır ekler.

Değişen tek eski iddia ekrandaki cümledir ve o bir SI satırında değil,
