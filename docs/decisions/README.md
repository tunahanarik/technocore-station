...
| IMP-366 | `OUTBOUND_CLIENT_MODULES` düz kümeden **`{dizin: {modül}}` haritasına** dönüştü | Eski yazım (`name in MODULES and parent.name == "technocore"`) iki yönden yanlıştı: `opencode/client.py` httpx import etseydi **geçerdi** — `client.py` zaten listedeydi — ve aynı anda her izinli modülün tek bir dizinde yaşadığını iddia ediyordu. Harita her giden yüzeyi kendi dizininde adlandırır ve bir dizindeki izinli adı başka dizine taşımaz |
...
| IMP-374 | `MODEL_MAPPINGS`'in her satırı `unverified`; **bu sürümde hiçbir model seçilemez** ve bunu bir test sabitler | Belge hangi modelin hangi protokol ailesine ait olduğunu söylemiyor. Uydurmak, yanlış aileye giden ve kullanıcının hatası gibi görünen bir 4xx üretirdi. Boş ama gerekçeli bir liste, dolu ama tahmini bir listeden dürüsttür; mekanizma enjekte edilen bir test tablosuyla uçtan uca kanıtlanır |
