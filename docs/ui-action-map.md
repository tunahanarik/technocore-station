...
| "Baglantiyi denetle" | anahtar kayitli **ve** model secili **ve** denetim tavani dolmamis **ve** baska islem ucusta degil; aksi halde buton disabled | `POST /api/opencode/check` (`checkOpenCodeConnection`, **135 sn**) - **bu uygulamadaki tek ucretli istek** | buton "Denetleniyor..." + hepsi disabled | **gercek bir istek gider ve saglayicinin cevabi oldugu gibi gosterilir** (ADR-0015): secili modele `chat/completions` uzerinden tek bir kucuk cagri (`max_tokens: 16`, arac registry si yok). Sonuc dort durumdan biri - `verified` (yesil; tek ureticisi olculu uctan gelen bir `200`), `provider_refused`, `probe_failed`, ya da hic sorulmadiysa `key_saved_unverified` - arti gerekcelerin **tamami**, arti "Son denetim" ve "Denetim hakki n/8". Probe un yanlisladigi gerekceler **dusurulur**. Sayfa acilisinda, pollda veya anahtar kaydedilirken **yapilmaz** | `ErrorRegion` "Baglanti denetlenemedi" - sunucu duzyazisi **korunur** (istek kullanicinin yazdigi hicbir seyi tasimaz; gelen mesajlar "model secilmedi" gibi okunmasi gereken retlerdir); saglayicinin anahtari reddetmesi **hata degildir**, belge icinde hukum olarak doner | `kind=timeout` ayni bolge; istemci suresi sunucunun 120 sn okuma tavaninin ustundedir ki yerel bir kronometre "reddedildi" ile "ogrenemedik" ayrimini ezmesin | yok | `OpenCodeConnectionPanel.test.tsx::sends the check to its own endpoint and shows what came back`, `::shows a refusal as a refusal and never as the verdict before it`, `::disables the check while it is in flight and does not send a second one`, `::greys the check out once the ceiling for this key is spent`, `::never calls the metered check on mount`, `test_opencode_probe.py::test_a_saved_key_can_reach_a_verified_verdict_when_the_provider_accepts_it` |
...
- **Ücretli probe butonu vardır ve tam olarak birdir** (ADR-0015: "Baglantiyi denetle"). Basılmadan çalışmaz, sayılır ve kimlik bilgisi başına tavanı vardır. Aşağıdaki paragraf yazıldığı gün doğruydu ve gerekçesi hâlâ doğrudur; değişen sonucudur — gerçek küçük bir çağrının maliyetli
  olabileceği yazılır **ve kullanıcının açık eylemine bağlıdır** — düğmenin
  kendisi o açık eylemdir. "Bu turda uygulanmaz" (ADR-0005 §4) satırının
  arkasında beş paket boyunca hiçbir şey yoktu; ADR-0015 onu kapattı.
...
| Ne | Nerede | Test |
|---|---|---|
| Çıkarımın bedeli: satırları bir model okur ve yanılabilir (ADR-0014; eski satır "anlamsal çıkarım yok" diyordu) | "Bu taramanin siniri" bölümü, **her** okumada, sonuç olmadan da | `workscan-honesty` |
| Taramanın maliyeti: tur başına satır ve tarama başına tur sayısı, **harcanmadan önce** | aynı bölüm, dürüstlük cümlesinin altında, koşulsuz | `workscan-reading-cost` |
| Taramanın **harcadığı** tur sayısı ve tavanı | tarama sonucu bloğu | `workscan-model-calls` |
